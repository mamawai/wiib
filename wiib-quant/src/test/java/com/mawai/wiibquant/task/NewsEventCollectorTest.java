 ...
 * 下轮自愈——硬插空标会让这批永远失去打标机会）、正常路标签随行落库。
 */
class NewsEventCollectorTest {
    ...
    void 新快讯打标后落库且标签与模型名随行() {
        ...
        when(tagger.tag(any(), anyList(), anyList())).thenReturn(Map.of(1L, "BTC"));

        collector().collect();

        verify(mapper).insertIgnore(eq(1L), eq("BTC 突破十万"), eq("正文"), anyString(),
                anyLong(), eq("BTC"), eq("light-model"));
    }
    ...
    void 已入库的快讯不再打标不再插() {
        ...
        when(tagger.tag(any(), anyList(), anyList())).thenReturn(Map.of(2L, ""));

        collector().collect();

        // 只有新的那条进打标与落库；打标调用的名单也只有它——存量白烧打标的钱正是要防的
        verify(tagger).tag(any(), eq(List.of(flash(2, "新闻"))), anyList());
        verify(mapper, never()).insertIgnore(eq(1L), anyString(), anyString(), anyString(),
                anyLong(), anyString(), anyString());
        verify(mapper).insertIgnore(eq(2L), anyString(), anyString(), anyString(),
                anyLong(), eq(""), anyString());
    }
    ...
    void 打标失败本轮一条都不插() {
        ...
        verify(mapper, never()).insertIgnore(anyLong(), anyString(), anyString(), anyString(),
                anyLong(), anyString(), anyString());
    }
