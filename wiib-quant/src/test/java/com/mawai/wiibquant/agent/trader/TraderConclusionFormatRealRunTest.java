 ...
 * <pre>
 * WIIB_REAL_RUN=1 mvn test -pl wiib-quant -am -DskipTests=false \
...
class TraderConclusionFormatRealRunTest {
    ...
    void 新收尾格式两门语言真模型各服从一轮() {
        ...
        for (AgentLang lang : AgentLang.values()) {
            ...
            String instruction = runner.routineInstruction(trader, boundary, snapshot, lang, false)
                    + (lang == AgentLang.ZH
            ...
            List<ReviewMaterialAssembler.ConclusionSegment> segments =
                    ReviewMaterialAssembler.splitSegments(output.substring(output.lastIndexOf(mark) + mark.length()));
            assertThat(segments.stream().map(ReviewMaterialAssembler.ConclusionSegment::symbol))
