 ...
 * 这里只管缓存策略。
 * <p>
 * 缓存键是<b>配置指纹</b>，userId 是它的第一个分量：一个用户一份由键本身保证，
 * 改端点指纹就变、自然拿到新实例，不需要显式 evict。
 * <b>userId 那一格不能省</b>：叶子里有按用户烤死的工具，两人共享一份叶子就是跨用户泄露。
 * 重新输入 key（哪怕同一把）会因 IV 变化触发一次重建，旧实例被 LRU 淘汰，无实际损失。
 */
@Slf4j
@Component
