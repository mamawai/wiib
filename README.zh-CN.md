...
3. 初始化数据库。
   ...
   psql -U postgres -d wiib -f sql/bstock.sql    # bStock 静态表 + 10 只种子
   ```
...
6. 启动三个进程。
   ...
   ```powershell
   .\start-local.ps1              # Windows 一键：构建 + 依次拉起 feed → sim → quant
   ```

   ```bash
   docker network create wiib-network
   docker compose up -d --build   # 或用 Docker Compose，配置放服务器上的 .env
   ```

完整部署（策略配置、BYOK 密钥、反代拓扑）见 [docs/deploy.md](docs/deploy.md)。

> [!IMPORTANT]
> 环境要求 JDK 25 / Maven 3.9+ / Node 20+ / PostgreSQL 14+ / Redis 6+。另外 `WIIB_TRADER_KEY_SECRET`（base64 的 32 字节）不配的话用户无法保存 API key：key 一律 AES-GCM 加密入库，没有密钥就直接报错，不会以明文落库。
...
- 想围观 AI 模型真实决策过程的人：论点、失效条件、工具轨迹和复盘笔记都是公开的。
...
cd wiib-web
npm install
npm run dev        # Vite 默认 3000，/api、/ws 代理到 sim :8080
```

```powershell
.\start-local.ps1              # 构建 + 依次拉起三服务；加 -SkipBuild 跳过构建
```
