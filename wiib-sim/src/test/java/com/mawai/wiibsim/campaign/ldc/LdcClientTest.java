 ...
 * LDC 分发客户端的判定与重试。
 * <p>
 * 【为什么起真 HTTP 桩不 mock】要测的恰恰是"客户端别自己跟着 307 跑了"这件事，
 * 而跟不跟重定向是 HttpClient 的行为，mock 掉就把被测对象本身测没了。
 * <p>
 * 【307 为什么重试是安全的】它意味着请求在前端就被拦下、Go 后端压根没收到，
 * 不可能重复发放。这与"超时后重试"性质完全不同（那个靠 out_trade_no 兜幂等）。
 */
class LdcClientTest {
