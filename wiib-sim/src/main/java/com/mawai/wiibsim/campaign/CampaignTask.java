...
public class CampaignTask {
     ...
     * <p>
     * 【要盖住的是 15 个投票日】活动 14 天，但投票日盖的是 UTC 日戳、活动窗口按服务器本地时间开合，
     * 于是能落到 vote_date 上的 UTC 日有 15 个（种子活动是 08-03 ~ 08-17，
     * 推导见 CampaignVoteService.poolOf）。只要有一轮把这 15 天整个盖住，
     * 哪怕整场的定时任务全没跑成，跑通那一次也能把所有日子补齐，不用人肉介入。
     * <p>
     * 【为什么是 17 不是 15】上面那句"跑通一次就够"取决于那一轮落在哪一晚，两头都要够：
     * 尾巴 —— 票投的是明天，末个投票日 08-17 要到 UTC 08-18 才算过完，所以 今天−1 ≥ 08-17，即 今天 ≥ 08-18；
     * 头 —— 今天−SWEEP_DAYS ≤ 08-03，即 今天 ≤ 08-03 + SWEEP_DAYS。
     * 取 15 时两头正好卡死成一个点（今天 = 08-18），偏一晚（08-19）头就够不着 08-03 了，
     * 兜底得指望"全场唯一那次跑通恰好发生在 08-18"。取 17 后 08-18/19/20 三晚里任意一晚跑通都能一轮补齐 ——
     * 多出来的这 2 天买的就是"补跑那晚自己也可能没跑成"的余量。改活动天数的话这个数要跟着改：
     * 小了两头补不全，大了只是多几次空查。
     * <p>
     * 【为什么这么扫不贵】settleDay 头一件事就是查该日有没有待结算的票，没有就直接返回，
     * 连行情都不取。正常那一晚是 16 次空查 + 1 次真结算。
     */
