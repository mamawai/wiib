...
| 块 | 来源 | 要点 |
|---|---|---|
| 账户状态 | `accountStateJson` | 持仓带计划与修订历史、挂单带已挂时长、待确认请求与已处理结果各回注一次。一次给足，工具预算才能留给行情求证 |
...
- **审批不阻塞**：`allowSelfAdd/Reduce` 关掉时，工具调用转成 `TraderRequestService` 的一行待确认请求，落库即返回——本轮该干的其余动作照干，结果下一轮才在账户状态里露面。
